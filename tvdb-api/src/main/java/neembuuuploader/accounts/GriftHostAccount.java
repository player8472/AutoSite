 ...
 * @author Paralytic
 */
public class GriftHostAccount extends AbstractAccount{
    ...
    public GriftHostAccount() {
        KEY_USERNAME = "grifthostusername";
        KEY_PASSWORD = "grifthostpassword";
        HOSTNAME = "GriftHost.com";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().griftHostCheckBox.setEnabled(false);
        HostsPanel.getInstance().griftHostCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to GriftHost.com");
            httpPost = new NUHttpPost("http://grifthost.com");

            List<NameValuePair> formparams = new ArrayList<NameValuePair>();
            formparams.add(new BasicNameValuePair("op", "login"));
            formparams.add(new BasicNameValuePair("redirect", "http://grifthost.com/?op=my_files"));
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfss").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().griftHostCheckBox.setEnabled(true);
                NULogger.getLogger().info("GriftHost.com login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("GriftHostAccount.html", responseString);
