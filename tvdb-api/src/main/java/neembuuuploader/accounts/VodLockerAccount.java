 ...
 * @author Paralytic
 */
public class VodLockerAccount extends AbstractAccount{
    ...
    public VodLockerAccount() {
        KEY_USERNAME = "vodlckrusername";
        KEY_PASSWORD = "vodlckrpassword";
        HOSTNAME = "VodLocker.com";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().vodLockerCheckBox.setEnabled(false);
        HostsPanel.getInstance().vodLockerCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to VodLocker.com");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfsts").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().vodLockerCheckBox.setEnabled(true);
                NULogger.getLogger().info("VodLocker.com login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("VodLockerAccount.html", responseString);
