 ...
 */
public class AsfileAccount extends AbstractAccount{
    ...
    public AsfileAccount() {
        KEY_USERNAME = "asflusername";
        KEY_PASSWORD = "asflpassword";
        HOSTNAME = "Asfile.com";
    }
    ...
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().asFileCheckBox.setEnabled(false);
        HostsPanel.getInstance().asFileCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to Asfile.com");
            httpPost = new NUHttpPost("http://asfile.com/en/login");
            ...
            if (lastHeader != null) {
                ...
                if(responseString.contains("You have Limited access.")){
                    premium = false;
                }
                else{
                    premium = true;
                ...
                NULogger.getLogger().log(Level.INFO, "Asfile premium: {0}", premium);
                
                loginsuccessful = true;
                username = getUsername();
                password = getPassword();
                HostsPanel.getInstance().asFileCheckBox.setEnabled(true);
                
                
                NULogger.getLogger().info("Asfile.com login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("AsfileAccount.html", responseString);
                Document doc = Jsoup.parse(responseString);
                String error = doc.getElementById("wrap").text();
                
                if(error.contains("Fail login")){
