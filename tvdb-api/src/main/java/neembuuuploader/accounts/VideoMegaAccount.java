 ...
 * @author Paralytic
 */
public class VideoMegaAccount extends AbstractAccount{
    ...
    public VideoMegaAccount() {
        KEY_USERNAME = "vdmegausername";
        KEY_PASSWORD = "vdmegapassword";
        HOSTNAME = "VideoMega.tv";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().videoMegaCheckBox.setEnabled(false);
        HostsPanel.getInstance().videoMegaCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to VideoMega.tv");
            httpPost = new NUHttpPost("http://videomega.tv/index_login.php");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "uid").isEmpty() && !CookieUtils.getCookieValue(httpContext, "pwd").isEmpty()) {
                EntityUtils.consume(httpResponse.getEntity());
                loginsuccessful = true;
                username = getUsername();
                password = getPassword();
                HostsPanel.getInstance().videoMegaCheckBox.setEnabled(true);
                NULogger.getLogger().info("VideoMega.tv login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("VideoMegaAccount.html", responseString);
