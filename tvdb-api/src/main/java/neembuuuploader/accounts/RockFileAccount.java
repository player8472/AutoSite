 ...
 * @author Paralytic
 */
public class RockFileAccount extends AbstractAccount{
    ...
    public RockFileAccount() {
        KEY_USERNAME = "rckflusername";
        KEY_PASSWORD = "rckflpassword";
        HOSTNAME = "RockFile.eu";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().rockFileCheckBox.setEnabled(false);
        HostsPanel.getInstance().rockFileCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to RockFile.eu");
            httpPost = new NUHttpPost("https://rockfile.eu/login");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfss").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().rockFileCheckBox.setEnabled(true);
                NULogger.getLogger().info("RockFile.eu login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("RockFileAccount.html", responseString);
