 ...
 * @author Paralytic
 */
public class NovaFileAccount extends AbstractAccount{
    ...
    public NovaFileAccount() {
        KEY_USERNAME = "novaflusername";
        KEY_PASSWORD = "novaflpassword";
        HOSTNAME = "NovaFile.com";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().novaFileCheckBox.setEnabled(false);
        HostsPanel.getInstance().novaFileCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to NovaFile.com");
            httpPost = new NUHttpPost("http://novafile.com/login");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfss").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().novaFileCheckBox.setEnabled(true);
                NULogger.getLogger().info("NovaFile.com login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("NovaFileAccount.html", responseString);
