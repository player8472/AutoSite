 ...
 * @author Paralytic
 */
public class UpSharedAccount extends AbstractAccount{
    ...
    public UpSharedAccount() {
        KEY_USERNAME = "upshrdusername";
        KEY_PASSWORD = "upshrdpassword";
        HOSTNAME = "UpShared.com";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().upSharedCheckBox.setEnabled(false);
        HostsPanel.getInstance().upSharedCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to UpShared.com");
            httpPost = new NUHttpPost("http://upshared.com");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfss").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().upSharedCheckBox.setEnabled(true);
                NULogger.getLogger().info("UpShared.com login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("UpSharedAccount.html", responseString);
