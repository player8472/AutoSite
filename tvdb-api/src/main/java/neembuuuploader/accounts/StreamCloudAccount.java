 ...
 * @author Paralytic
 */
public class StreamCloudAccount extends AbstractAccount{
    ...
    public StreamCloudAccount() {
        KEY_USERNAME = "strmcldusername";
        KEY_PASSWORD = "strmcldpassword";
        HOSTNAME = "StreamCloud.eu";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().streamCloudCheckBox.setEnabled(false);
        HostsPanel.getInstance().streamCloudCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to StreamCloud.eu");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfsts").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().streamCloudCheckBox.setEnabled(true);
                NULogger.getLogger().info("StreamCloud.eu login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("StreamCloudAccount.html", responseString);
    ...
    private void initialize() throws Exception {
        ...
        //NULogger.getLogger().info("Getting startup cookies & link from StreamCloud.eu");
        //responseString = NUHttpClientUtils.getData("http://streamcloud.eu", httpContext);
    }
