 ...
 * @author Paralytic
 */
public class TeraFileAccount extends AbstractAccount{
    ...
    public TeraFileAccount() {
        KEY_USERNAME = "terafusername";
        KEY_PASSWORD = "terafpassword";
        HOSTNAME = "TeraFile.co";
    }
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to TeraFile.co");
            httpPost = new NUHttpPost("http://terafile.co");
			
            List<NameValuePair> formparams = new ArrayList<NameValuePair>();
            formparams.add(new BasicNameValuePair("op", "login"));
            formparams.add(new BasicNameValuePair("redirect", "http://terafile.co/"));
            ...
            if (httpResponse != null && httpResponse.toString().contains("xfss=")) {
                ...
                NULogger.getLogger().info("TeraFile.co login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("TeraFileAccount.html", responseString);
