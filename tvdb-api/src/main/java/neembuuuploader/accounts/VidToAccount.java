 ...
 * @author Paralytic
 */
public class VidToAccount extends AbstractAccount{
    ...
    public VidToAccount() {
        KEY_USERNAME = "vidtousername";
        KEY_PASSWORD = "vidtopassword";
        HOSTNAME = "VidTo.me";
    }
    ...
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().vidToCheckBox.setEnabled(false);
        HostsPanel.getInstance().vidToCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to VidTo.me");
            httpPost = new NUHttpPost("http://vidto.me/");

            List<NameValuePair> formparams = new ArrayList<NameValuePair>();
            formparams.add(new BasicNameValuePair("op", "login"));
            formparams.add(new BasicNameValuePair("redirect", "http://vidto.me/"));
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfsts").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().vidToCheckBox.setEnabled(true);
                NULogger.getLogger().info("VidTo.me login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("VidToAccount.html", responseString);
