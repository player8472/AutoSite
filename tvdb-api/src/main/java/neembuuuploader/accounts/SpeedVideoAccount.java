 ...
 * @author Paralytic
 */
public class SpeedVideoAccount extends AbstractAccount{
    ...
    public SpeedVideoAccount() {
        KEY_USERNAME = "spdvidusername";
        KEY_PASSWORD = "spdvidpassword";
        HOSTNAME = "SpeedVideo.net";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().speedVideoCheckBox.setEnabled(false);
        HostsPanel.getInstance().speedVideoCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to SpeedVideo.net");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfsts").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().speedVideoCheckBox.setEnabled(true);
                NULogger.getLogger().info("SpeedVideo.net login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("SpeedVideoAccount.html", responseString);
