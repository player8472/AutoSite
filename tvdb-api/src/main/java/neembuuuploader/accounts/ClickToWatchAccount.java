 ...
 * @author Paralytic
 */
public class ClickToWatchAccount extends AbstractAccount{
    ...
    public ClickToWatchAccount() {
        KEY_USERNAME = "clktowtchusername";
        KEY_PASSWORD = "clktowtchpassword";
        HOSTNAME = "ClickToWatch.net";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().clickToWatchCheckBox.setEnabled(false);
        HostsPanel.getInstance().clickToWatchCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to ClickToWatch.net");
            httpPost = new NUHttpPost("http://clicktowatch.net");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfss").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().clickToWatchCheckBox.setEnabled(true);
                NULogger.getLogger().info("ClickToWatch.net login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("ClickToWatchAccount.html", responseString);
