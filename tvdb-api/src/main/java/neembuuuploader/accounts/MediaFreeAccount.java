 ...
 * @author Paralytic
 */
public class MediaFreeAccount extends AbstractAccount{
    ...
    public MediaFreeAccount() {
        KEY_USERNAME = "mediafrusername";
        KEY_PASSWORD = "mediafrpassword";
        HOSTNAME = "MediaFree.co";
    }
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to MediaFree.co");
            httpPost = new NUHttpPost("http://mediafree.co/");

            List<NameValuePair> formparams = new ArrayList<NameValuePair>();
            formparams.add(new BasicNameValuePair("op", "login"));
            formparams.add(new BasicNameValuePair("redirect", "http://mediafree.co/"));
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfss").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                NULogger.getLogger().info("MediaFree.co login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("MediaFreeAccount.html", responseString);
    ...
    private void initialize() throws Exception {
        ...
        //NULogger.getLogger().info("Getting startup cookies & link from MediaFree.co");
