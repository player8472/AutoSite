 ...
 */
public class LomaFileAccount extends AbstractAccount{
    ...
    public LomaFileAccount() {
        KEY_USERNAME = "lmflusername";
        KEY_PASSWORD = "lmflpassword";
        HOSTNAME = "Lomafile.com";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().lomaFileCheckBox.setEnabled(false);
        HostsPanel.getInstance().lomaFileCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to LomaFile.com");
            ...
            if (lastHeader != null && lastHeader.getValue().contains("op=my_files")) {
                ...
                responseString = NUHttpClientUtils.getData("http://lomafile.com/?op=my_account", httpContext);
                
                Document doc = Jsoup.parse(responseString);
                
                if(doc.select("td.account_block table td").eq(4).text().equals("Limited")){
                    premium = false;
                }
                else{
                    premium = true;
                ...
                NULogger.getLogger().log(Level.INFO, "LomaFile premium: {0}", premium);
                
                loginsuccessful = true;
                username = getUsername();
                password = getPassword();
                HostsPanel.getInstance().lomaFileCheckBox.setEnabled(true);
                NULogger.getLogger().info("LomaFile.com login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("LomaFileAccount.html", responseString);
                Document doc = Jsoup.parse(responseString);
