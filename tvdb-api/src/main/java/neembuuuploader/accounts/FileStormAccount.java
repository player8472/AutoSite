 ...
 * @author Paralytic
 */
public class FileStormAccount extends AbstractAccount{
    ...
    public FileStormAccount() {
        KEY_USERNAME = "flstormusername";
        KEY_PASSWORD = "flstormpassword";
        HOSTNAME = "FileStorm.to";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().fileStormCheckBox.setEnabled(false);
        HostsPanel.getInstance().fileStormCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to FileStorm.to");
            httpPost = new NUHttpPost("http://filestorm.to");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfss").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().fileStormCheckBox.setEnabled(true);
                NULogger.getLogger().info("FileStorm.to login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("FileStormAccount.html", responseString);
