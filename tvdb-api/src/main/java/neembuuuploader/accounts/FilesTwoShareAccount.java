 ...
 * @author Paralytic
 */
public class FilesTwoShareAccount extends AbstractAccount{
    ...
    public FilesTwoShareAccount() {
        KEY_USERNAME = "flstoshrusername";
        KEY_PASSWORD = "flstoshrpassword";
        HOSTNAME = "Files2Share.ch";
    }

    @Override
    public void disableLogin() {
        resetLogin();
        HostsPanel.getInstance().filesTwoShareCheckBox.setEnabled(false);
        HostsPanel.getInstance().filesTwoShareCheckBox.setSelected(false);
    ...
    public void login() {
        ...
        try {
            ...
            NULogger.getLogger().info("Trying to log in to Files2Share.ch");
            ...
            if (!CookieUtils.getCookieValue(httpContext, "xfss").isEmpty() && !CookieUtils.getCookieValue(httpContext, "login").isEmpty()) {
                ...
                HostsPanel.getInstance().filesTwoShareCheckBox.setEnabled(true);
                NULogger.getLogger().info("Files2Share.ch login successful!");

            } else {
                //Get error message
                responseString = EntityUtils.toString(httpResponse.getEntity());
                //FileUtils.saveInFile("FilesTwoShareAccount.html", responseString);
