 ...
 * @author Paralytic
 */
public class GBoxes extends AbstractUploader{
    
    GBoxesAccount gBoxesAccount = (GBoxesAccount) AccountsManager.getAccount("GBoxes.com");
    ...
    public GBoxes(File file) {
        super(file);
        downURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        delURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        host = "GBoxes.com";
        if (gBoxesAccount.loginsuccessful) {
            host = gBoxesAccount.username + " | GBoxes.com";
        }
    ...
    public void run() {
        try {
            if (gBoxesAccount.loginsuccessful) {
                userType = "reg";
                httpContext = gBoxesAccount.getHttpContext();
            ...
            // http://s5.gboxes.com/cgi-bin/upload.cgi?upload_id=509848328736&js_on=1&utype=reg&upload_type=file
            ...
            NULogger.getLogger().info("Now uploading your file into GBoxes.com");
            uploading();
            ...
            if (upload_fn != null) {
                httpPost = new NUHttpPost("http://www.gboxes.com/");
