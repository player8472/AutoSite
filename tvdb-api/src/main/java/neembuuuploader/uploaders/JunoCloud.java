 ...
 */
public class JunoCloud extends AbstractUploader{
    
    JunoCloudAccount junoCloudAccount = (JunoCloudAccount) AccountsManager.getAccount("JunoCloud.me");
    ...
    public JunoCloud(File file) {
        super(file);
        downURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        delURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        host = "JunoCloud.me";
        if (junoCloudAccount.loginsuccessful) {
            host = junoCloudAccount.username + " | JunoCloud.me";
        }
        maxFileSizeLimit = 4294967296l; //4096 MB
    ...
    public void run() {
        try {
            if (junoCloudAccount.loginsuccessful) {
                userType = "reg";
                httpContext = junoCloudAccount.getHttpContext();
                sessionID = CookieUtils.getCookieValue(httpContext, "xfss");
                
                if(junoCloudAccount.isPremium()){
                    maxFileSizeLimit = 21474836480l; //20480 MB
                }
                else{
                    maxFileSizeLimit = 4294967296l; //4096 MB
                }
            ...
            NULogger.getLogger().info("Now uploading your file into JunoCloud.me");
            uploading();
            ...
            //FileUtils.saveInFile("JunoCloud.html", responseString);
