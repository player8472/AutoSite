    ...
    public void run() {
        try {
            ...
            if (file.length() > fileSizeLimit) {
                JOptionPane.showMessageDialog(neembuuuploader.NeembuuUploader.getInstance(), "<html><b>" + getClass().getSimpleName() + "</b> " + TranslationProvider.get("neembuuuploader.exceptions.maxfilesize") + ": <b>1040MB</b></html>", getClass().getSimpleName(), JOptionPane.ERROR_MESSAGE);
                uploadInvalid();
                return;
            }
            uploadInitialising();
