 ...
 * @author Paralytic
 */
public class CloudFly extends AbstractUploader{
    
    CloudFlyAccount cloudFlyAccount = (CloudFlyAccount) AccountsManager.getAccount("CloudFly.us");
    ...
    public CloudFly(File file) {
        super(file);
        downURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        delURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        host = "CloudFly.us";
        if (cloudFlyAccount.loginsuccessful) {
            host = cloudFlyAccount.username + " | CloudFly.us";
        }
    ...
    private void initialize() throws Exception {
        responseString = NUHttpClientUtils.getData("http://cloudfly.us/", httpContext);
    ...
    public void run() {
        try {
            if (cloudFlyAccount.loginsuccessful) {
                userType = "reg";
                httpContext = cloudFlyAccount.getHttpContext();
            ...
            // http://files.cloudfly.us/cgi-bin/upload.cgi?upload_id=210441050498&js_on=1&utype=reg&upload_type=file
            ...
            NULogger.getLogger().info("Now uploading your file into CloudFly.us");
            uploading();
            ...
            if (upload_fn != null) {
                httpPost = new NUHttpPost("http://cloudfly.us/");
