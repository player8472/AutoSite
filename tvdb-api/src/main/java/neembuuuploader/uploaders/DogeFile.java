...
public class DogeFile extends AbstractUploader{
    ...
    DogeFileAccount dogeFileAccount = (DogeFileAccount) AccountsManager.getAccount("DogeFile.com");
    ...
    public DogeFile(File file) {
        super(file);
        downURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        delURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        host = "DogeFile.com";
        if (dogeFileAccount.loginsuccessful) {
            host = dogeFileAccount.username + " | DogeFile.com";
        }
        maxFileSizeLimit = 104857600; // 100 MB (default)
    }

    private void initialize() throws Exception {
        responseString = NUHttpClientUtils.getData("http://dogefile.com", httpContext);
    ...
    public void run() {
        try {
            if (dogeFileAccount.loginsuccessful) {
                userType = "reg";
                httpContext = dogeFileAccount.getHttpContext();
            ...
            // http://50.115.126.92/~dogefile/cgi-bin/upload.cgi?upload_id=969972189047&js_on=1&utype=reg&upload_type=file
            ...
            NULogger.getLogger().info("Now uploading your file into DogeFile.com");
            uploading();
            ...
            if (upload_fn != null) {
                httpPost = new NUHttpPost("http://dogefile.com/");
