 ...
 * @author Paralytic
 */
public class UploadRocket extends AbstractUploader{
    
    UploadRocketAccount uploadRocketAccount = (UploadRocketAccount) AccountsManager.getAccount("UploadRocket.net");
    ...
    public UploadRocket(File file) {
        super(file);
        downURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        delURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        host = "UploadRocket.net";
        if (uploadRocketAccount.loginsuccessful) {
            host = uploadRocketAccount.username + " | UploadRocket.net";
        }
    ...
    public void run() {
        try {
            if (uploadRocketAccount.loginsuccessful) {
                userType = "reg";
                httpContext = uploadRocketAccount.getHttpContext();
                maxFileSizeLimit = 6442450944L; // 6 GB
            } else {
                userType = "anon";
            ...
            // http://fs005.uploadrocket.net/cgi-bin/upload.cgi?upload_id=446503822480&js_on=1&utype=reg&upload_type=file
            ...
            NULogger.getLogger().info("Now uploading your file into UploadRocket.net");
            uploading();
            ...
            if (upload_fn != null) {
                httpPost = new NUHttpPost("http://uploadrocket.net/");
