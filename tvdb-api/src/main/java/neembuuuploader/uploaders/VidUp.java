 ...
 * @author Paralytic
 */
public class VidUp extends AbstractUploader{
    
    VidUpAccount vidUpAccount = (VidUpAccount) AccountsManager.getAccount("VidUp.me");
    ...
    public VidUp(File file) {
        super(file);
        downURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        delURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        host = "VidUp.me";
        if (vidUpAccount.loginsuccessful) {
            host = vidUpAccount.username + " | VidUp.me";
        }
        maxFileSizeLimit = 419430400L; // 400 MB (default)
        
    }

    private void initialize() throws Exception {
        responseString = NUHttpClientUtils.getData("http://vidup.me", httpContext);
    ...
    public void run() {
        try {
            if (vidUpAccount.loginsuccessful) {
                userType = "reg";
                httpContext = vidUpAccount.getHttpContext();
            ...
            // http://84.39.117.115/cgi-bin/upload.cgi?upload_id=789937414402&js_on=1&utype=reg&upload_type=file
            ...
            NULogger.getLogger().info("Now uploading your file into VidUp.me");
            uploading();
            ...
            if (upload_fn != null) {
                httpPost = new NUHttpPost("http://vidup.me");
     ...
     * var ext_allowed='AVI|MP4|MKV|DIVX|M4A|FLV';
     */
