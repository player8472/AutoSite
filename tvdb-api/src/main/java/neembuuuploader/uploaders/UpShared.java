...
public class UpShared extends AbstractUploader implements UploaderAccountNecessary{
    ...
    UpSharedAccount upSharedAccount = (UpSharedAccount) AccountsManager.getAccount("UpShared.com");
    ...
    public UpShared(File file) {
        super(file);
        downURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        delURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        host = "UpShared.com";
        if (upSharedAccount.loginsuccessful) {
            host = upSharedAccount.username + " | UpShared.com";
        }
        maxFileSizeLimit = 1258291200L; // 1,200 MB  (default)
    ...
    public void run() {
        try {
            if (upSharedAccount.loginsuccessful) {
                userType = "reg";
                httpContext = upSharedAccount.getHttpContext();
                sessionID = CookieUtils.getCookieValue(httpContext, "xfss");
                maxFileSizeLimit = 1258291200L; // 1,200 MB
            } else {
                host = "UpShared.com";
                uploadInvalid();
                return;
            ...
            // http://upshared.com/cgi-bin/upload.cgi?upload_id=092984936122&js_on=1&utype=reg&upload_type=file
            ...
            NULogger.getLogger().info("Now uploading your file into UpShared.com");
            uploading();
