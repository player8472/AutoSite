...
public class SafeSharing extends AbstractUploader{
    ...
    SafeSharingAccount safeSharingAccount = (SafeSharingAccount) AccountsManager.getAccount("SafeSharing.eu");
    ...
    public SafeSharing(File file) {
        super(file);
        downURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        delURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        host = "SafeSharing.eu";
        if (safeSharingAccount.loginsuccessful) {
            host = safeSharingAccount.username + " | SafeSharing.eu";
        }
        maxFileSizeLimit = 10485760; // 10 MB (default)
    ...
    private void initialize() throws Exception {
        ...
        m_e = doc.select("form").first().select("input[name=m_e]").attr("value");
    }
    ...
    public void run() {
        try {
            ...
            if (upload_fn != null) {
                ...
                doc = Jsoup.parse(responseString);
                downloadlink = doc.select("textarea").eq(1).val();
