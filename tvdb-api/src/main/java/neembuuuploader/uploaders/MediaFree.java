 ...
 * @author Paralytic
 */
public class MediaFree extends AbstractUploader{
    
    MediaFreeAccount mediaFreeAccount = (MediaFreeAccount) AccountsManager.getAccount("MediaFree.co");
    ...
    public MediaFree(File file) {
        super(file);
        downURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        delURL = UploadStatus.PLEASEWAIT.getLocaleSpecificString();
        host = "MediaFree.co";
        if (mediaFreeAccount.loginsuccessful) {
            host = mediaFreeAccount.username + " | MediaFree.co";
        }
        maxFileSizeLimit = 2147483648L; // 2 GB (default)
    }

    private void initialize() throws Exception {
        responseString = NUHttpClientUtils.getData("http://mediafree.co", httpContext);
    ...
    public void run() {
        try {
            if (mediaFreeAccount.loginsuccessful) {
                userType = "reg";
                httpContext = mediaFreeAccount.getHttpContext();
            ...
            // http://files2.mediafree.co/cgi-bin/upload.cgi?upload_id=658207955461&js_on=1&utype=reg&upload_type=file
            ...
            NULogger.getLogger().info("Now uploading your file into MediaFree.co");
            uploading();
            ...
            if (upload_fn != null) {
                httpPost = new NUHttpPost("http://mediafree.co/");
