...
public class QueueManager {
    ...
    public void updateQueue() {
        ...
        for (int i = 0; i < NUTableModel.uploadList.size(); i++) {
            Uploader uploader = NUTableModel.uploadList.get(i);
            ...
            if (uploader.getStatus() == UploadStatus.QUEUED) {
                ...
                try {
                    ...
                    Constructor<? extends Uploader> uploaderConstructor = uploader.getClass().getConstructor(File.class);
    ...
    public void startNextUploadIfAny() {
        ...
        if (getQueuedUploadCount() == 0 && currentlyUploading == 0){
            ...
            if(SystemTray.getSystemTray().getTrayIcons().length > 0) {
                SystemTray.getSystemTray().getTrayIcons()[0].displayMessage(TranslationProvider.get("neembuuuploader.NeembuuUploader.neembuuuploader"), TranslationProvider.get("neembuuuploader.QueueManager.SystemTray.allUploadsCompleted"), TrayIcon.MessageType.INFO);
            }
        } else {
            updateQueue();
        }
    ...
    public int getQueuedUploadCount() {
        ...
        for (int i = 0; i < NUTableModel.uploadList.size(); i++) {
            ...
            if (UploadStatusUtils.isRowStatusOneOf(i, UploadStatus.QUEUED) || 
                    (SettingsProperties.isPropertyTrue("autoretryfaileduploads") && 
    ...
    private void setFrameTitle(){
        if(SettingsProperties.isPropertyTrue("showoverallprogress")){
            if(getQueuedUploadCount() == 0 && currentlyUploading == 0){
                NeembuuUploader.getInstance().resetTitle();
            }
            else{
                int averageProgress = getUploadProgressPercentage();
                NeembuuUploader.getInstance().setTitle( averageProgress + "% " + TranslationProvider.get("neembuuuploader.NeembuuUploader.neembuuuploader"));
            }
        }
    }
